...
const MobileReader = ({ 
  ...
  visibleNovelsCount: number,
  novelsEndRef: (node?: Element | null | undefined) => void
}) => {
  const [readerView, setReaderView] = useState<'home' | 'novel-details' | 'reading'>('home');
  const [selectedChapterAction, setSelectedChapterAction] = useState<Chapter | null>(null);
          ...
          {readerView === 'home' && (
            ...
            >
              ...
              {filteredNovels.length > visibleNovelsCount && (
                <div ref={novelsEndRef} className="mt-10 flex justify-center py-6">
                  <div className="flex items-center gap-3 text-white/20">
                    <Loader2 className="w-5 h-5 animate-spin" />
                    <span className="text-[10px] font-bold">جاري تحميل المزيد...</span>
                  </div>
                </div>
              )}
            </motion.div>
  ...
  if (mode === 'reader') {
    return (
      <MobileReader 
        ...
        visibleNovelsCount={visibleNovelsCount}
        novelsEndRef={novelsEndRef}
      />
    );
  }
